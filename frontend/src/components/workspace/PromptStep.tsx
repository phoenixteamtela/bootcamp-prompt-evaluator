      ...
      <div style={{ background: colors.white, borderRadius: 12, border: `1px solid ${colors.gray[200]}`, padding: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <h2 style={{ margin: 0, fontSize: 16, color: colors.navy, display: 'flex', alignItems: 'center' }}>
            ...
            {isEditing && hasVersions && <Tooltip text="Give this version a short name so you can remember what you changed, like 'added examples' or 'more specific instructions.'" />}
          </h2>
        ...
        {!hasVersions && (
          <p style={{ fontSize: 13, color: colors.gray[500], margin: '0 0 12px', lineHeight: 1.5 }}>
            Tell the AI what to do. This is the instruction it will follow for every test case.
            The test case data (like {Object.keys(project.prompt_inputs_spec ?? {}).map(k => `"${k}"`).join(', ')}) is automatically provided — just focus on describing the task.
          </p>
        )}
        {hasVersions && (
        ...
        <textarea
          ...
          readOnly={!isEditing}
          rows={!hasVersions ? 14 : 12}
          placeholder={!hasVersions ? 'Write your instructions here...' : undefined}
          style={{
        ...
        {isEditing && (
          <div style={{ display: 'flex', gap: 8, marginTop: 12, justifyContent: 'flex-end' }}>
            {hasVersions && <button onClick={() => setIsEditing(false)} style={btnStyle('secondary')}>Cancel</button>}
            <button onClick={handleSave} style={btnStyle('primary')}>
              {hasVersions ? 'Save New Version' : 'Save Prompt'}
            </button>
          </div>
        )}
      </div>
