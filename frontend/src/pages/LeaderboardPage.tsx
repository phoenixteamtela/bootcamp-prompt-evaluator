...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    const url = isGlobal ? '/api/leaderboard' : `/api/projects/${projectId}/leaderboard`;
    api.get<LeaderboardEntry[]>(url).then(setEntries).finally(() => setLoading(false));
  }, [projectId, isGlobal]);

  return (
    <div>
      <h1 style={{ color: colors.navy, fontSize: 24, marginBottom: 24 }}>
            ...
            <tbody>
              {entries.map(entry => (
                <tr key={entry.user_id} style={{ borderBottom: `1px solid ${colors.gray[100]}` }}>
                  ...
                  {isGlobal && <td style={{ padding: '12px 16px', fontSize: 13 }}>{entry.project_name}</td>}
