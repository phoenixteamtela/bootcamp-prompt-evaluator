...
export default function NewProjectPage() {
  ...
  const [inputs, setInputs] = useState<{ key: string; description: string }[]>([
    { key: '', description: '' },
  ]);
  const [error, setError] = useState('');
  const [saving, setSaving] = useState(false);

  const addInput = () => setInputs([...inputs, { key: '', description: '' }]);
  const removeInput = (i: number) => setInputs(inputs.filter((_, idx) => idx !== i));
  const updateInput = (i: number, field: 'key' | 'description', value: string) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    for (const input of inputs) {
      ...
      spec[input.key.trim()] = input.description.trim();
    }
    if (Object.keys(spec).length === 0) {
      setError('At least one prompt input is required');
      return;
    }
    setSaving(true);
    try {
